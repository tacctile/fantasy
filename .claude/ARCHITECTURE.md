...
src/
  ...
  services/
    sleeper/        — server-only Sleeper integration (Wave 2); ESPN later gets the structurally separate sibling services/espn/ (no shared code path)
      client.ts     — sleeperGet<T>() + SleeperApiError: typed HTTP client, backoff/retry per sleeper-api/rate-limits (named exports)
      types.ts      — permissive wire types (SleeperPlayerRecord, SleeperPlayersCatalog); unrecognized fields preserved
...
- **Scripts:** `dev` (turbopack), `build`, `start`, `lint`, `gen:types` (regenerates `src/lib/supabase/database.types.ts` from the linked project — the canonical regeneration command after any schema migration), `sync:players` (manual Sleeper player-catalog sync via `tsx --env-file=.env.local` — at most once per day per sleeper-api guidance)
...
**`services/sleeper/types.ts`** — `SleeperPlayerRecord` (every field optional + unknown-preserving index signature — Sleeper's field inventory is unversioned), `SleeperPlayersCatalog` (map keyed by `player_id`, not an array).
