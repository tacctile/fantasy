...
**Current status:** **Wave 5 (`05_eye_candy.md`) is now 🟡 in progress** — its Shared foundations sub-section shipped 2026-07-31 (4 items, one artifact fold): the chart layer every Wave 5 section mounts on, with no feature section built on it yet. Four decisions of record from that fold: no charting library (hand-rolled SVG/CSS primitives in `src/components/charts/`, zero client JS so both surfaces stay server-rendered), a machine-validated teal-free categorical ramp (`--chart-1` no longer aliases `--primary`), `components/charts/` shared by the admin AND spectator surfaces (the first exemption to the separate-rendering-path rule), and `RosterSlotLayout`/`parseRosterSlotLayout` extracted into the new `services/league-context.ts` (re-exported by `draft-board.ts`, consumers untouched). Next: the **Score charts** sub-section. Wave 4 🟢 complete (2026-07-31). Wave 3b 🟢 complete. Wave 3c 🔴 ESPN-blocked (~mid-August 2026). Wave 3a 🟡 bye-week `[>]` schedule-gated. Wave 2 🟡 ESPN residue `[!]`, cron cadence `[~]`, finality promotion `[>]`. Still standing: run the waiver-formula scoping session in plain chat before Wave 5's waiver/FAAB item.
