...
**Last Updated:** 2026-07-22 (Wave 4 build progressing: data-access layer, freshness surfacing, the four display components, and now the page-assembly + league-selector fold are all `[x]` — the admin dashboard is mounted at `/leagues/[leagueId]` with root auto-land re-signed onto it. Next unchecked items: the nav-shell sub-section; the share-link panel rides the share-token RLS singleton. Standing context: Nick's wave-order ruling runs Wave 4 ahead of ESPN-blocked 3b; the scoring-engine sub-section was cut by Nick-signed correctness amendment (`player_scores` is ingested platform-scored, never computed). Prior state: Wave 3a 🟡 — all five sub-sections complete, sole remainder the schedule-source-gated bye-week `[>]`; `02_data_pipeline.md` 🟡: 9 ESPN items `[!]` external-timing, cron cadence `[~]`, finality promotion `[>]`; Wave 1 complete — `01_foundation.md` 🟢)
...
**Dependency notes:** Wave 3b depends on Wave 2's ESPN integration being live and isolated (its failure must not break Wave 3a's static board or Sleeper-sourced features). Wave 3a can start once Wave 1's schema and Wave 2's Sleeper sync are done — it does not need ESPN. Wave 4 does not depend on 3b or ESPN — Nick authorized it to run ahead of ESPN-blocked 3b (2026-07-22 ruling); its data needs are served by the live Sleeper pipeline, and ESPN leagues join its surfaces automatically once connected. 3b's position in roadmap order is unchanged — it resumes when the ESPN block clears.
