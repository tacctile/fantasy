...
**Current status:** **Wave 5 (`05_eye_candy.md`) is 🟡 in progress** — Shared foundations shipped 2026-07-31 (4 items) and **Score charts shipped the same day** (7 items, one artifact fold): Wave 5's first real feature is live at `/leagues/[leagueId]/score-trends`, its own route lighting the sidebar slot Wave 4 reserved (Nick's Clarify; item 7's wording amended to match, since it predated the nav shell). Decisions of record from the foundations fold: no charting library (hand-rolled SVG/CSS primitives in `src/components/charts/`, zero client JS so both surfaces stay server-rendered), a machine-validated teal-free categorical ramp (`--chart-1` no longer aliases `--primary`), `components/charts/` shared by the admin AND spectator surfaces (the first exemption to the separate-rendering-path rule), and `RosterSlotLayout`/`parseRosterSlotLayout` extracted into the new `services/league-context.ts` (re-exported by `draft-board.ts`, consumers untouched). From Score charts: a week's points is the stored `matchups.effective_points`, never re-summed from `player_scores`; week scope is regular-season-only through the SAME `parsePlayoffWeekStart` power rankings uses (now exported from `dashboard.ts`, not copied); and the ≤4-series cap lives inside the section, fanning out to small multiples past it. Next: the **Lucky/unlucky tracker** sub-section. Wave 4 🟢 complete (2026-07-31). Wave 3b 🟢 complete. Wave 3c 🔴 ESPN-blocked (~mid-August 2026). Wave 3a 🟡 bye-week `[>]` schedule-gated. Wave 2 🟡 ESPN residue `[!]`, cron cadence `[~]`, finality promotion `[>]`. Still standing: run the waiver-formula scoping session in plain chat before Wave 5's waiver/FAAB item.
...
| 5 | Eye Candy | 🟡 | Score charts (🟢 shipped 2026-07-31 — own route `/leagues/[id]/score-trends`), lucky/unlucky tracker, positional breakdowns, playoff picture, trade evaluation, waiver/FAAB recommendations (trade + waiver amended into scope 2026-07-21 — see 05_eye_candy.md scope note; waiver scoring formula itself deferred to a follow-up session) |
