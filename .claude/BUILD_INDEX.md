...
**Current status:** **Wave 5 (`05_eye_candy.md`) is 🟡 in progress** — three sub-sections shipped 2026-07-31: Shared foundations (4 items), Score charts (7 items) at `/leagues/[leagueId]/score-trends`, and the **Lucky/unlucky tracker** (7 items, one artifact fold) at `/leagues/[leagueId]/luck`, which lights the second of the sidebar slots Wave 4 reserved. Decisions of record from the luck fold (all Nick's Clarify, 2026-07-31): actual records are RECOMPUTED from regular-season matchup pairings rather than read from the `standings` snapshot, so both sides of the differential share one week scope (the snapshot rides along and any disagreement is disclosed); a scored week with no opponent rates for the all-play expectation but records no game, so `weeksRated` can exceed `gamesPlayed` and that asymmetry is stated on the surface rather than smoothed away; a sub-6-week reading is flagged, never suppressed; and Pythagorean expectation is deliberately NOT computed, because the wiki records that no proposed exponent is corroborated. Two decisions were declared genuine wiki silence at decision time: ranking tie-breakers (which follow the Nick-signed standings-ordering shape) and luck magnitude bands (so the spectator's short label is derived from sign alone). Item 7's spectator summary shipped as a COMPONENT only — its share-surface wiring stays with the Integration sub-section, which owns the share-token-scoped query. Decisions of record from the foundations fold: no charting library (hand-rolled SVG/CSS primitives in `src/components/charts/`, zero client JS so both surfaces stay server-rendered), a machine-validated teal-free categorical ramp (`--chart-1` no longer aliases `--primary`), `components/charts/` shared by the admin AND spectator surfaces (the first exemption to the separate-rendering-path rule), and `RosterSlotLayout`/`parseRosterSlotLayout` extracted into the new `services/league-context.ts` (re-exported by `draft-board.ts`, consumers untouched). From Score charts: a week's points is the stored `matchups.effective_points`, never re-summed from `player_scores`; week scope is regular-season-only through the SAME `parsePlayoffWeekStart` power rankings uses (now exported from `dashboard.ts`, not copied); and the ≤4-series cap lives inside the section, fanning out to small multiples past it. Next: the **Positional breakdowns** sub-section (5 items) — a new wiki category (`league-mechanics/roster-construction-starting-lineups`), so a legitimate fold boundary. Wave 4 🟢 complete (2026-07-31). Wave 3b 🟢 complete. Wave 3c 🔴 ESPN-blocked (~mid-August 2026). Wave 3a 🟡 bye-week `[>]` schedule-gated. Wave 2 🟡 ESPN residue `[!]`, cron cadence `[~]`, finality promotion `[>]`. Still standing: run the waiver-formula scoping session in plain chat before Wave 5's waiver/FAAB item.
...
| 5 | Eye Candy | 🟡 | Score charts (🟢 shipped 2026-07-31 — own route `/leagues/[id]/score-trends`), lucky/unlucky tracker (🟢 shipped 2026-07-31 — own route `/leagues/[id]/luck`), positional breakdowns, playoff picture, trade evaluation, waiver/FAAB recommendations (trade + waiver amended into scope 2026-07-21 — see 05_eye_candy.md scope note; waiver scoring formula itself deferred to a follow-up session) |
