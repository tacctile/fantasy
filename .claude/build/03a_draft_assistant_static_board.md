...
- [ ] Build a server-only ADP ingestion service that fetches Sleeper's public ADP data, validates the response shape, and normalizes rows against canonical `sleeper_player_id` (no separate identity system, per Schema Rule #4)
