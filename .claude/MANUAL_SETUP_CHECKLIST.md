...
- [ ] **Confirm the real ADP source before ADP ingestion is built.** No Sleeper API wiki page documents a public ADP endpoint — verify one actually exists before the build session writes ingestion against it; if not, decide a real alternative source (e.g. FantasyPros) and obtain any API key it requires. — *Verification DONE 2026-07-22 (plain chat, live-tested; decision still open — awaiting Nick's pick). Findings: Sleeper's documented API has NO ADP endpoint, but the undocumented projections endpoint `api.sleeper.com/projections/nfl/{year}?season_type=regular` carries per-format ADP fields (`adp_ppr`/`adp_half_ppr`/`adp_std`/`adp_2qb`/`adp_dynasty*`/`adp_rookie`) keyed by native `sleeper_player_id` — 2026 data confirmed live (no auth, no key; undocumented = could change without notice, so ingestion gets ESPN-grade defensive isolation, which 03a's items already require; no dispersion/sample-size metadata; population parameters unpublished). Alternatives verified live: FantasyFootballCalculator (free REST, no key, attribution requested, once-daily update, explicit sample metadata + per-player stdev/high/low, league-size param — but proprietary player IDs only → name-based mapping, the wiki's last-resort join); FantasyPros (key by request; free prototype tier bundled with MVP/HOF membership ~$5.99/mo annual; personal non-commercial production keys for members; FP player IDs → extra mapping layer). Rejected with wiki grounding: Underdog (best-ball ADP, non-transferable to managed redraft per league-mechanics/average-draft-position) and ESPN ADP (fragile platform + autodraft/default-contaminated casual population per the same page). Claude's recommendation: the Sleeper undocumented endpoint — population-matched to the platform Nick's league actually drafts on, native sleeper_player_id (zero mapping), zero cost. One-word pick from Nick flips this item and fully unblocks Wave 3a.*
