...
- [x] **Confirm the real ADP source before ADP ingestion is built.** No Sleeper API wiki page documents a public ADP endpoint — verify one actually exists before the build session writes ingestion against it; if not, decide a real alternative source (e.g. FantasyPros) and obtain any API key it requires. — *Resolved 2026-07-22 (plain chat, Nick's decision of record): ADP SOURCE = SLEEPER, via the undocumented projections endpoint `api.sleeper.com/projections/nfl/{year}?season_type=regular` — live-verified same day: per-format ADP fields (`adp_ppr`/`adp_half_ppr`/`adp_std`/`adp_2qb`/`adp_dynasty*`/`adp_rookie`) keyed by native `sleeper_player_id` (sample: Bijan Robinson `"9509"`, adp_ppr 1.4; 2026 data live). No API key — nothing left to hand off. Constraints the build session must honor: (1) undocumented endpoint → ESPN-grade defensive isolation + last-good-snapshot preservation (03a's items already spec both); (2) it lives on `api.sleeper.com`, NOT the documented `api.sleeper.app/v1` host the existing client is pinned to; (3) no dispersion/sample-size metadata and unpublished population parameters — accepted; (4) wiki coverage RESOLVED 2026-07-22 (same-day dedicated wiki-maintenance session): the endpoint is now covered by `wiki/topics/sleeper-api/projections-endpoint.md` — the source of record for shape, ADP field inventory, and defensive posture (live re-verified during ingestion; notably: response is a JSON *array* not an ID-keyed map, ADP fields sit inside `stats`, a 12th field `adp_dynasty_2qb` exists beyond the list recorded here, and `999.0` is a not-ranked sentinel to treat as null). The ADP build session includes that page in its wiki reads. Alternatives verified and passed over: FantasyFootballCalculator (free/no-key, has stdev+sample metadata, but proprietary player IDs → name-based mapping, the wiki's last-resort join); FantasyPros (key-gated, ~$5.99/mo tier, FP IDs → extra mapping). Rejected with wiki grounding: Underdog (best-ball ADP non-transferable to managed redraft) and ESPN ADP (fragile platform + autodraft-contaminated population), both per league-mechanics/average-draft-position.*
- [x] Confirm which of your real leagues (from Wave 2) should populate the board. — *Resolved 2026-07-22 (plain chat, Nick's decision): ALL CONNECTED LEAGUES — the board populates from the `leagues` table with no hardcoded count. Today that is the one connected Sleeper league (2025 "10 enter 1 Leaves", `1257464256710856704`, season-complete — Nick confirmed it acceptable as the initial build-and-verify board target); the 2026 Sleeper renewal joins when minted; ESPN leagues join automatically once connected (~mid-August). Confirmed: no both-platform requirement for Wave 3a to proceed (Sleeper-only is sufficient per 03a scope + BUILD_INDEX dependency notes).*
...
- **2026-07-22 — Wave 4 authorized ahead of ESPN-blocked 3b; scoring-engine sub-section cut (Nick).** With 01/02/03a holding zero `[ ]` items and 3b ESPN-blocked (~mid-Aug), Nick ruled builds proceed into Wave 4 (League Dashboard) now — its data needs are served by the live Sleeper pipeline; ESPN leagues join its surfaces automatically when connected; 3b's roadmap position is unchanged and resumes when ESPN opens. Same session, Nick signed the correctness amendment resolving `04_league_dashboard.md`'s scoring-engine sub-section against Wave 2's shipped architecture: `player_scores` is ingested platform-scored (Sleeper `players_points`, league-scored at source; ESPN as-received when unblocked) — items 1–4 `[-]` cut, item 5 re-worded to surfacing the existing `fetched_at`/`is_final` freshness machinery, and the FA-week gap (no score rows for weeks a player was unrostered — absent from every platform payload) accepted for v1 with honest not-rostered rendering. Backsweep applied to `02_data_pipeline.md`'s exclusions line the same session.
