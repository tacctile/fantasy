...
| ESPN API | `wiki/topics/espn-api/` | ESPN platform integration reference: cookie-based auth (espn_s2/SWID), undocumented view parameters, rate limits, and endpoint structure for rosters, draft state, standings, and matchups. In progress — 13 of 19 subjects ingested (auth origin/mechanics, extraction/refresh, cookie/header format, public/private detection, auth-failure error responses, public/private data completeness, base URL/versioning, view parameter reference, roster response structure, matchup response structure, draft detail response structure, player endpoint and filtering, historical season access). |
