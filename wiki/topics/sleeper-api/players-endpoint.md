...
Field presence is uneven across records — treat every field beyond `player_id` as potentially absent, especially for inactive, historical, or deep practice-squad players. Commonly present fields include: `first_name`, `last_name`, `full_name` (frequently absent or unreliable for defenses); `position` (primary position) and `fantasy_positions` (an array — a player can be fantasy-eligible at more than one position simultaneously); `team` (the current NFL team abbreviation, `null` for a free agent); `status` (roster status such as Active, Inactive, Injured Reserve, or Practice Squad) and `injury_status` (a separate field for the official injury-report designation — the full semantics, including bye-week retention behavior and the gap between Sleeper IR-slot eligibility and real NFL injured reserve, are covered on `sleeper-api/player-data-quirks`); `age`, `height`, `weight`, `college`, `years_exp`, and `birth_date`; `number` (jersey number); `depth_chart_position` and `depth_chart_order`; `search_full_name` (a lowercased, punctuation-stripped normalization of the player's name that Sleeper uses internally for search, and the best available field for any fuzzy name-based lookup); and `search_rank` (Sleeper's internal relevance/display ordering — low values for actively relevant fantasy players, very high sentinel-style values for deep or inactive entries). This is Sleeper's own internal ordering signal, not a talent or value metric, and it is not a documented, stable contract. There is no `bye_week` field anywhere in this record — see `sleeper-api/player-data-quirks` for why byes must always be derived from team-and-schedule data instead.
...
### Caching Architecture, Refresh Validation, and Client-Side Storage Pitfalls

The correct architecture is a single, centrally-scheduled ingestion job — not one fetch per worker, per process, or per user session. If several independently-scheduled workers each pull on their own timer, Sleeper receives redundant large calls and different workers can end up holding different snapshots at the same moment; one job publishing one shared snapshot avoids both problems and matches the vendor's own "fetch once daily, save it yourself" guidance.

A robust refresh should be atomic rather than an in-place overwrite: validate the new response before swapping it in as the current snapshot, and never let a failed or malformed refresh destroy the last known-good copy. Practical validation checks include confirming the top-level value is an object (not a truncated body or an HTML error page), that the record count is plausible relative to recent history, and that a representative sample of records still carries the expected core fields. Retaining the raw snapshot itself (not just a normalized index built from it) is also worth doing — it preserves fields normalization may have dropped and allows reprocessing after a schema change without waiting for the next scheduled fetch.

Two client-side pitfalls are worth calling out explicitly. First, browser `localStorage` quotas commonly sit around 5MB per origin — at or below the dump's own documented size — so the full payload should never be cached there; if any client-side caching is needed at all, IndexedDB or the Cache Storage API are the appropriate mechanisms, and a server-owned cache serving only the subset of fields a given client actually needs is preferable to shipping the full dump to any client in the first place. Second, parsing a multi-megabyte JSON blob on every cold invocation is a real latency and memory cost on constrained runtimes — serverless functions with cold starts and on-device mobile parsing both pay this cost repeatedly if the full dump is re-parsed per invocation rather than served from a pre-built, already-normalized cache.

The documented ~5MB average payload size should itself be treated as an approximate historical figure rather than a hard ceiling to design against — real-world reports describe the payload continuing to grow as the tracked player pool expands, and compressed transfer size, uncompressed JSON size, and parsed in-memory size are three meaningfully different numbers that are easy to conflate. Measuring actual size against a live production pull is more reliable than hard-coding any specific byte figure, including the one cited on this page.
...
- **Decision:** The platform will run a single centrally-scheduled ingestion job that validates a new response (object shape, plausible record count, sample-field presence, non-truncated body) before atomically swapping it in as the current snapshot, and will never cache the full players dump in browser `localStorage`.
  **Reasoning:** Multiple independently-scheduled fetchers produce redundant load and inconsistent snapshots across workers; an unvalidated in-place overwrite risks replacing a good snapshot with a malformed or partial one; and browser storage quotas sit at or below the payload's own documented size, making `localStorage` structurally unsuitable for it.
  **Rejected alternative:** Letting each worker or client fetch and cache independently was rejected — it multiplies external request volume for no freshness benefit and creates the possibility of different parts of the system operating against different snapshots at the same time.
...
- [ ] What is the current, complete field inventory and exact payload size at any given moment, given that fields are added without notice and size has grown over time? — needs verification against a fresh pull rather than trusting any static list, including this page's. Reports reviewed for this page ranged from the documented ~5MB average up to claims of a much larger uncompressed size; treat any single cited figure, including the ones on this page, as approximate.
