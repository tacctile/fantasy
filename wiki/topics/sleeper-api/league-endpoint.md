...
The exact key set present on a given league depends heavily on its format and the era in which it was created. Older leagues and newer formats do not carry an identical settings schema, and new keys have appeared over time without any formal migration notice or version bump. Sleeper's own published documentation shows only a skeletal placeholder for this object rather than an enumerated schema — the full key inventory used on this page is reconstructed entirely from cross-corroborated community observation of live league responses, not from an official source. An integration that hard-codes an expected key list will silently miss newer fields and should instead treat `settings` as an open map, surfacing recognized keys explicitly while preserving unrecognized ones rather than discarding them. `playoff_week_start` specifically is required to map a playoff bracket round to the correct NFL week — see `sleeper-api/playoff-bracket-endpoint`.
