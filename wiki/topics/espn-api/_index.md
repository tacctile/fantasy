...
| [breaking-changes-and-schema-drift](breaking-changes-and-schema-drift.md) | No versioning contract, categories of historical breakage (schema shifts, host migration, auth drift, enum changes, view-requirement changes), the silent-corruption failure mode, and season-rollover risk. | high |
| [bye-ir-and-lineup-lock-quirks](bye-ir-and-lineup-lock-quirks.md) | Bye status as a derived (not exposed) field, per-player kickoff-based lineup locking, and the IR-slot-retains-ineligible-player quirk. | high |
